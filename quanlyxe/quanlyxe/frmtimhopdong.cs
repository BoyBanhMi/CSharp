    ...
    public partial class frmtimhopdong : Form
    {
        ...
        private void btntim_Click(object sender, EventArgs e)
        {
            if (rbTimKiemTheoMaHD.Checked == true)
            {
                ...
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter("select MaHopDong as [Mã hợp đồng], TenHopDong as [Tên hợp đồng], NgayLapHopDong as [Ngày lập hợp đồng], HanThanhToan as [Ngày hết hạn], MaKhachHang as [Mã khách hàng], MaNhanVien as [Mã nhân viên],  TinhTrangThanhToan as [Tình trạng thanh toán] from tb_HopDong where MaHopDong like '%" + txtTimKiemHopDong.Text + "%'", con);
            ...
            if (rbTimKiemTheoTenHD.Checked == true)
            {
                SqlConnection con = new SqlConnection(Program.strconn);
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter("select MaHopDong as [Mã hợp đồng], TenHopDong as [Tên hợp đồng], NgayLapHopDong as [Ngày lập hợp đồng], HanThanhToan as [Ngày hết hạn], tb_HopDong.MaKhachHang as [Mã khách hàng], MaNhanVien as [Mã nhân viên],  TinhTrangThanhToan as [Tình trạng thanh toán] from tb_HopDong where TenHopDong like '%" + txtTimKiemHopDong.Text + "%'", con);
                DataSet ds = new DataSet();
                da.Fill(ds, "tb_HopDong");
                dgvTimKiemHopDong.DataSource = ds.Tables["tb_HopDong"].DefaultView;
            }
